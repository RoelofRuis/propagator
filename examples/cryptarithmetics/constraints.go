...
func (s Sum) Scope() []propagator.DomainId {
	aDomains := s.A.Scope()
	bDomains := s.B.Scope()
	sumDomains := s.Sum.Scope()
	res := make([]propagator.DomainId, len(aDomains)+len(bDomains)+len(sumDomains))
	copy(res[0:], aDomains)
...
func (n Number) Scope() []propagator.DomainId {
	return propagator.IdsOf(n.Variables...)
}
...
func (a AllDifferent) Scope() []propagator.DomainId {
	return propagator.IdsOf(a.Variables...)
}
