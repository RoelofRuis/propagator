...
func (h House) Scope() []propagator.DomainId {
...
func (c FixedCage) Scope() []propagator.DomainId {
	return propagator.IdsOf(c.Cell)
}
...
func (c SumCage) Scope() []propagator.DomainId {
	return propagator.IdsOf(c.Cells...)
}
...
func (c ProdCage) Scope() []propagator.DomainId {
	return propagator.IdsOf(c.Cells...)
}
...
func (c SubCage) Scope() []propagator.DomainId {
	if len(c.Cells) > 3 {
...
func (c DivCage) Scope() []propagator.DomainId {
	if len(c.Cells) > 2 {
		// FIXME: this assumes only 2 cells take part
